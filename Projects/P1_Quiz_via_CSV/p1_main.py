...
def individual_response(user_data, quiz_number, quiz_questions):
	...
	for q in quiz_questions:
		if(q.answer_correct == True):
			...
			Total[1] += 1
		elif(q.answer_correct == 'unattempted'):
			Total[2] += 1
...
def end_page(main_window, quiz_questions, user_data):
	for widget in main_window.winfo_children():
		widget.destroy()
	total = individual_response(user_data, quiz_number, quiz_questions)
	total_marks = total[3]
	tqq = len(quiz_questions)
	tqqa = tqq - total[2]
	tca = total[0]
	twa = total[1]
	table_2 = "Quiz_wise_marks"
	values_old = list((user_data[0], str(quiz_number), str(total_marks)))
	db = dbhandler(database_name, table_2, [("ROLL","TEXT", "PRIMARY KEY","NOT NULL"), ("QUIZ", "TEXT", "NOT NULL"), ("TOTAL", "TEXT", "NOT NULL")])
	db.insert(table_2, values_old, select_val = None)
	marks_list = [int(x.obtained_marks) for x in quiz_questions]
	total_marks = sum(marks_list)

	label_TQQ = tk.Label(
			main_window,
			text = "Total Quiz Questions: " + str(tqq),
			font = ("helvetica", 20),
			background = background_color,
			justify = "center")
	label_TQQA = tk.Label(
			main_window,
			text = "Total Quiz Questions Answered: " + str(tqqa),
			font = ("helvetica", 20),
			background = background_color,
			justify = "center")
	label_TCA = tk.Label(
			main_window,
			text = "Total Correct Answers: " + str(tca),
			font = ("helvetica", 20),
			background = background_color,
			justify = "center")
	label_TWA = tk.Label(
			main_window,
			text = "Total Wrong Answers: " + str(twa),
			font = ("helvetica", 20),
			background = background_color,
			justify = "center")
	label_M = tk.Label(
			main_window,
			text = "Total Marks " + str(total_marks),
			font = ("helvetica", 20),
			background = background_color,
			justify = "center")
	label_TQQ.pack(side = 'top', pady = (10,10), expand = 'True')
	label_TQQA.pack(side = 'top', pady = 10, expand = 'True')
	label_TCA.pack(side = 'top', pady = 10, expand = 'True')
	label_TWA.pack(side = 'top', pady = 10, expand = 'True')
	label_M.pack(side = 'top', pady = 10, expand = 'True')
	...
	button_exit.pack(side = 'top', pady = 10, expand = 'True')
	main_window.wait_window(label_TQQ)
	try:
		main_window.destroy()
	except:
		pass
...
def start_quiz(start_button, main_window, background_color, user_data):
	...
	frame_timer, label_unattempted = display(main_window, background_color, user_data, quiz_questions)
	...
	for question in quiz_questions:
		...
		button_exit = tk.Button(
			...
			font = ("helvetica", 14),
			justify = "left",
			command = partial(end_page, main_window, quiz_questions, user_data))
		button_exit.pack()
